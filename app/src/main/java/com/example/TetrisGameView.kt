...
class TetrisGameView @JvmOverloads constructor(
    ...
    init {
        nextPiece = generatePiece(); spawnPiece()
    ...
    private fun spawnPiece() {
        ...
        if (!isValidPosition(currentPiece!!.matrix, currentPiece!!.x, currentPiece!!.y)) {
            isGameOver = true; soundManager.playGameOver()
        }
    }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        val padding = w * 0.15f 
        boardSizeW = w - padding * 2; cellSize = boardSizeW / COLS
        boardSizeH = cellSize * ROWS; boardX = padding; boardY = 200f

        val controlCenterY = boardY + boardSizeH + 180f
        ...
        restartBtnRect.set(w/2f - 250f, height/2f + 50f, w/2f + 250f, height/2f + 180f)
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isGameOver) {
            ...
            draw3DControlButton(canvas, restartBtnRect, "RESTART", 0xFFFF5E62.toInt(), 0xFF8B0000.toInt())
        }
    }
    ...
    override fun onDetachedFromWindow() { super.onDetachedFromWindow(); handler.removeCallbacks(gameLoop); handler.removeCallbacks(renderLoop); soundManager.release() }
}
