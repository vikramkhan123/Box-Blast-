...
class AdventureGameView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    ...
    private var cellSize = 0f; private var boardSize = 0f
    ...
    private val renderLoop = object : Runnable {
        override fun run() { invalidate(); handler.postDelayed(this, 16L) }
    }
    ...
    private fun initLevel() {
        ...
        gemsCollected = 0
        isGameOver = false; isLevelComplete = false; isWaitingForAd = false; adCountdown = 5
            ...
            if (trayShapes[i] == null || trayShapes[i]!!.placed) {
                ...
                if (gemsCollected < targetGems && Random.nextFloat() < 0.35f) {
                    ...
                    if (validCoords.isNotEmpty()) { 
                        ...
                        copy[gr][gc] = getAvailableGemTypes().random() 
                    }
                }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        val padding = 30f 
        boardSize = w - padding * 2; cellSize = boardSize / 8
        boardX = padding; boardY = padding + 250f
    ...
    private fun updateTrayPositions() {
        ...
        for (i in 0 until 3) {
            ...
            if (shape != null && !shape.placed) {
                shape.cx = (i * sectionWidth) + (sectionWidth - (shape.cols * trayCellSize)) / 2f
                shape.cy = trayY + (sectionWidth - (shape.rows * trayCellSize)) / 2f
            }
        }
    }
    ...
    private fun drawGeminiBackground(canvas: Canvas) {
        ...
        // Fixed visible Background Blobs
        val cx1 = width / 2f + Math.sin(time / 2000.0).toFloat() * 250f
        val cy1 = height / 3f + Math.cos(time / 1500.0).toFloat() * 250f
        neonBgPaint.shader = RadialGradient(cx1, cy1, 800f, intArrayOf(0x66E94560, 0x00E94560), null, Shader.TileMode.CLAMP)
        canvas.drawRect(0f, 0f, width.toFloat(), height.toFloat(), neonBgPaint)

        val cx2 = width / 2f + Math.cos(time / 1800.0).toFloat() * 300f
        val cy2 = height / 1.5f + Math.sin(time / 2200.0).toFloat() * 300f
        neonBgPaint.shader = RadialGradient(cx2, cy2, 900f, intArrayOf(0x660F80FF, 0x000F80FF), null, Shader.TileMode.CLAMP)
        canvas.drawRect(0f, 0f, width.toFloat(), height.toFloat(), neonBgPaint)
    }

    private fun draw3DText(canvas: Canvas, text: String, x: Float, y: Float, mainColor: Int, depthColor: Int, size: Float) {
        text3DPaint.textSize = size
        text3DPaint.color = depthColor
        // Draw depth
        for (i in 1..6) canvas.drawText(text, x, y + i * 2, text3DPaint)
        // Draw Main
        text3DPaint.color = mainColor
        canvas.drawText(text, x, y, text3DPaint)
    }
            ...
            while (iterator.hasNext()) {
                ...
                if (gem.progress >= 1f) {
                    ...
                    if (gemsCollected >= targetGems && !isLevelComplete) {
                        isLevelComplete = true; soundManager.playVictory() 
                    }
    ...
    private fun drawGlassy3DBlock(canvas: Canvas, x: Float, y: Float, size: Float, colorId: Int) {
        ...
        if (colorId >= 10) {
            ...
            drawGemShape(canvas, x + size * 0.15f, y + size * 0.15f, size * 0.7f, colorId)
            return
        }
    ...
    // Dynamic Gem Drawer based on ID (10=Star, 11=Diamond, 12=Hexagon, 13=Heart)
    private fun drawGemShape(canvas: Canvas, x: Float, y: Float, size: Float, type: Int) {
        ...
        var colors = intArrayOf(0xFFFFFFA0.toInt(), 0xFFFFD700.toInt(), 0xFFE65C00.toInt()) // Default Gold
        ...
        when(type) {
            10 -> { // Star
                ...
                for (i in 0 until 10) {
                    ...
                    if (i == 0) path.moveTo(cx + cos(angle).toFloat() * r, cy + sin(angle).toFloat() * r)
                    else path.lineTo(cx + cos(angle).toFloat() * r, cy + sin(angle).toFloat() * r)
                }
                path.close()
            }
            11 -> { // Diamond
            ...
            13 -> { // Simple Heart
                ...
                path.moveTo(cx, y + size/4)
                path.cubicTo(x, y - size/4, x - size/2, cy, cx, y + size)
                path.moveTo(cx, y + size/4)
                path.cubicTo(x + size, y - size/4, x + size + size/2, cy, cx, y + size)
            }
        }
        ...
        starPaint.shader = null; starPaint.color = 0xAAFFFFFF.toInt()
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        if (event.action == MotionEvent.ACTION_DOWN) {
            if (isLevelComplete && centerBtnRect.contains(tx, ty)) {
                soundManager.playBtnClick(); currentLevel++; prefs.edit().putInt("AdventureLevel", currentLevel).apply(); initLevel(); return true
            }
            if (isWaitingForAd && centerBtnRect.contains(tx, ty)) {
                // Reward ad clicked -> Give 1 random single block in tray and continue
                soundManager.playBtnClick(); isWaitingForAd = false; handler.removeCallbacks(timerRunnable)
                trayShapes[0] = Shape(arrayOf(intArrayOf(1))); trayShapes[1] = null; trayShapes[2] = null; updateTrayPositions(); return true
            }
            if (isGameOver && centerBtnRect.contains(tx, ty)) {
                soundManager.playBtnClick(); initLevel(); return true
            }
        }
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                for (i in 0 until 3) {
                    val shape = trayShapes[i]
                    if (shape != null && !shape.placed) {
                        if (RectF(shape.cx - 30f, shape.cy - 30f, shape.cx + (shape.cols * trayCellSize) + 30f, shape.cy + (shape.rows * trayCellSize) + 30f).contains(tx, ty)) {
                            soundManager.playPick(); draggingShapeIndex = i; draggingShape = shape
                            shape.cx = tx - (shape.cols * cellSize) / 2f; shape.cy = ty - (shape.rows * cellSize) - 180f
                            dragTouchOffsetX = tx - shape.cx; dragTouchOffsetY = ty - shape.cy; return true
                        }
                    }
                }
            }
    ...
    private fun checkGameOverCondition() {
        ...
        if (!canMakeMove) { 
            isWaitingForAd = true; adCountdown = 5; handler.post(timerRunnable) 
        }
    }
