...
class GameView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    ...
    private fun checkLines() {
        ...
        if (total > 0) {
            ...
            val currentTheme = themes[(totalBlastsCounter / 10) % themes.size]
            ...
            for (r in rows) { 
                ...
                for (c in 0 until 8) { 
                    ...
                    // Generate 5 particles per block destroyed based on theme
                    for(i in 0..4) particles.add(createThemeParticle(blastX, blastY, currentTheme))
                    grid[r][c] = 0 
                } 
                score += 100 
            }
